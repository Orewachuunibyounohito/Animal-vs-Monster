...
namespace MyScripts.Tween{
    public class Tween : MonoBehaviour
    {
        ...
        public void MyTween( float duration, float from, float to, System.Action<float> func ){
            StartCoroutine( TweenRoutine( duration, from, to, func ) );
        }

        #region Main Tween
        private IEnumerator TweenRoutine( float duration, float from, float to, System.Action<float> func, float[] phaseTotal=null, float[] phaseDuration=null, float deltaTime=0.02f ){
        ...
        private bool CorrectValueArray( float[] values ){
            float total = 0f;
